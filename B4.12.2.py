...
def near_by_bd(user, session):
    ...
    athlete_id = None
    athlete_bd = None
    for id1, bd in athlete_id_bd.items():
        dist = abs(user_bd - bd)
...
def nearest_by_height(user, session):
    ...
    athlete_id = None
    athlete_height = None
    for id1, height in atlhete_id_height.items():
        if height is None:
            continue
        dist = abs(user_height - height)
        if not min_h or dist < min_h:
            min_h = dist
            athlete_id = id1
...
def main():
    ...
    if not user:
        ...
        print(
            "Ближайший по росту атлет: {}, рост: {}".format(height_athlete, height)
        )
